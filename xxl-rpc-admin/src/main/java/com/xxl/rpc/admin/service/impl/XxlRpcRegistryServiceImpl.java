...
public class XxlRpcRegistryServiceImpl implements IXxlRpcRegistryService, InitializingBean, DisposableBean {
    ...
    public ReturnT<String> update(XxlRpcRegistry xxlRpcRegistry) {
        ...
        // valid
        if (xxlRpcRegistry.getBiz() == null || xxlRpcRegistry.getBiz().trim().length() < 4
            || xxlRpcRegistry.getBiz().trim().length() > 255) {
            return new ReturnT<String>(ReturnT.FAIL_CODE, "业务线格式非法[4~255]");
        }
        if (xxlRpcRegistry.getEnv() == null || xxlRpcRegistry.getEnv().trim().length() < 2
            || xxlRpcRegistry.getEnv().trim().length() > 255) {
            return new ReturnT<String>(ReturnT.FAIL_CODE, "环境格式非法[2~255]");
        }
        if (xxlRpcRegistry.getKey() == null || xxlRpcRegistry.getKey().trim().length() < 4
            || xxlRpcRegistry.getKey().trim().length() > 255) {
    ...
    public ReturnT<String> add(XxlRpcRegistry xxlRpcRegistry) {
        ...
        // valid exist
        XxlRpcRegistry exist =
            xxlRpcRegistryDao.load(xxlRpcRegistry.getBiz(), xxlRpcRegistry.getEnv(), xxlRpcRegistry.getKey());
        if (exist != null) {
    ...
    @Override
    public ReturnT<String> registry(String accessToken, String biz, String env,
        ...
        for (XxlRpcRegistryData registryData : registryDataList) {
            if (registryData.getKey() == null || registryData.getKey().trim().length() < 4
                || registryData.getKey().trim().length() > 255) {
                return new ReturnT<String>(ReturnT.FAIL_CODE, "Registry Key Invalid[4~255]");
            }
            if (registryData.getValue() == null || registryData.getValue().trim().length() < 4
                || registryData.getValue().trim().length() > 255) {
    ...
    public DeferredResult<ReturnT<String>> monitor(String accessToken, String biz, String env, List<String> keys) {
        ...
        DeferredResult deferredResult =
            new DeferredResult(30 * 1000L, new ReturnT<>(ReturnT.SUCCESS_CODE, "Monitor timeout, no key updated."));
    ...
    private void checkRegistryDataAndSendMessage(XxlRpcRegistryData xxlRpcRegistryData) {
        // data json
        List<XxlRpcRegistryData> xxlRpcRegistryDataList = xxlRpcRegistryDataDao.findData(xxlRpcRegistryData.getBiz(),
            xxlRpcRegistryData.getEnv(), xxlRpcRegistryData.getKey());
        ...
        // update registry and message
        XxlRpcRegistry xxlRpcRegistry = xxlRpcRegistryDao.load(xxlRpcRegistryData.getBiz(), xxlRpcRegistryData.getEnv(),
                    ...
                    while (!executorStoped) {
                        try {
                            ...
                            if (xxlRpcRegistryData != null) {
                                ...
                                // delete
                                xxlRpcRegistryDataDao.deleteDataValue(xxlRpcRegistryData.getBiz(),
                                    xxlRpcRegistryData.getEnv(), xxlRpcRegistryData.getKey(),
                                    xxlRpcRegistryData.getValue());
            ...
            public void run() {
                while (!executorStoped) {
                    try {
                        ...
                        List<XxlRpcRegistryMessage> messageList =
                            xxlRpcRegistryMessageDao.findMessage(readedMessageIds);
                        if (messageList != null && messageList.size() > 0) {
                            for (XxlRpcRegistryMessage message : messageList) {
                                readedMessageIds.add(message.getId());

                                if (message.getType() == 0) { // from registry、add、update、deelete，ne need sync from db,
                                                              // only write

                                    XxlRpcRegistry xxlRpcRegistry =
                                        JacksonUtil.readValue(message.getData(), XxlRpcRegistry.class);
            ...
            public void run() {
                while (!executorStoped) {
                    ...
                    try {
                        ...
                        List<XxlRpcRegistry> registryList =
                            xxlRpcRegistryDao.pageList(offset, pagesize, null, null, null);
                        while (registryList != null && registryList.size() > 0) {
                            ...
                            for (XxlRpcRegistry registryItem : registryList) {
                                ...
                                if (registryItem.getStatus() == 1) {
                                    ...
                                    List<XxlRpcRegistryData> xxlRpcRegistryDataList = xxlRpcRegistryDataDao
                                        .findData(registryItem.getBiz(), registryItem.getEnv(), registryItem.getKey());
    ...
    private String parseRegistryDataFileName(String biz, String env, String key) {
        // fileName
        String fileName = registryDataFilePath.concat(File.separator).concat(biz).concat(File.separator).concat(env)
    ...
    public String setFileRegistryData(XxlRpcRegistry xxlRpcRegistry) {
        ...
        // fileName
        String fileName =
            parseRegistryDataFileName(xxlRpcRegistry.getBiz(), xxlRpcRegistry.getEnv(), xxlRpcRegistry.getKey());

        // valid repeat update
        Properties existProp = PropUtil.loadProp(fileName);
        if (existProp != null && existProp.getProperty("data").equals(xxlRpcRegistry.getData())
